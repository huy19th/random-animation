...
export class BrickBreaker {
	...
	constructor(windowSize: WindowSize, settings = BrickBreakerSettings.value) {
	...
	createBricks(settings: typeof BrickBreakerSettings.value) {
		...
		for (let i = 0; i < totalBrickVertically; i++) {
			const lightBrickCount =
				Math.floor(totalBrickHorizontally / 2) - 5 + Math.floor(Math.random() * 11);
